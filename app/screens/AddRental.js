...
import { VEHICLE_TYPE, VEHICLE_CATEGORY} from './constants';    
import InsertRentalModal from './InsertRentalModal';
...
class AddRental extends React.Component {
    ...
    constructor(props) {
      ...
      toggleInsertEmployeeModal = () => {
        this.setState({ isInsertRentalModalOpen: !this.state.isInsertRentalModalOpen });
      }
    
      getData = () => {
        this.setState({ errorMessage: "", loading: true })
        fetch('http://10.182.153.235:8000/api/availableVehicles', {
          method: "GET"
        })
          .then(res => res.text())
          .then(res => {
    ...
    render(){
        return (
          ...
          <Text style={{fontSize: 20, textAlign: 'center', marginBottom: 7}}>Available Vehicles</Text>
          {/* <TextInput 
          ...
          {/* Code for listing starts from here */}
                ...
                {this.state.vehicleRecords.map((data, index) => <View
                    ...
                    <Text style={styles.listItem}>Year: {data.Year}</Text>
                    <Text style={styles.listItem}>Type: {VEHICLE_TYPE[data.Type]}</Text>
                    <Text style={styles.listItem}>Category: {VEHICLE_CATEGORY[data.Category]}</Text>
                    <Text style={styles.listItem}>Daily: {data.Daily}</Text>
                    <Text style={styles.listItem}>Weekly: {data.Weekly}</Text>

                    <View style={styles.buttonContainer}>
                    <TouchableOpacity
                        onPress={() => {
                        this.toggleInsertEmployeeModal();
...
{this.state.isInsertRentalModalOpen ? <InsertRentalModal
            isOpen={this.state.isInsertRentalModalOpen}
            closeModal={this.toggleInsertEmployeeModal}
